...
namespace Microsoft.MSRC.Portal.JavascriptResxGenerator
{
	class Program
	{
        ...
        private static string regionToken = "en-us";
		...
		{            
			...
			foreach (var item in resxFiles)
			{
				...
				string javascriptOutDir = String.Format("{0}\\{1}.{2}.js", outDir, resourceNamespace, regionToken);
				...
				if (fileNameWithoutExtension.Contains("."))
				{
					...
					regionToken = fileNameWithoutExtension.Substring(fileNameWithoutExtension.IndexOf('.') + 1);
		...
		private static void GenerateJavaScriptResxFile(string resxFilePath, string outDir, string vsdocOutDir, string resourceNamespace, string registerNamespace, bool generateVsDoc)
		{
            ...
            var lowerCaseCulture = regionToken.ToLowerInvariant();
			...
			{
                var namespaces = registerNamespace.Split('.');
                var tempNs = "";
                registerNamespace = "";
                foreach (var ns in namespaces)
                {
                    tempNs = string.IsNullOrEmpty(tempNs) ? ns : tempNs + "." + ns;
                    registerNamespace += string.Format("\t {0} = {0} || {1}; {2}", tempNs, "{}", Environment.NewLine);
                }
				registerNamespace += string.Format("\t {0} = {0} || {1}; {2}", resourceNamespace, "{}", Environment.NewLine);
                registerNamespace += string.Format("\t {0}['{1}'] = {0}['{1}'] || {2}; {3} \t", resourceNamespace, lowerCaseCulture, "{}", Environment.NewLine);
            }

            var resourceNamespaceWithCulture = string.Format("{0}['{1}']", resourceNamespace, lowerCaseCulture);
            var resourceFileContents = _JavascriptFileTemplate.Replace("#KEYS#", resourceSb.ToString().TrimEnd(',')).Replace("#REGISTERNAMESPACE#", registerNamespace).Replace("#NAMESPACE#", resourceNamespaceWithCulture);
			File.WriteAllText(outDir, resourceFileContents);

			if (generateVsDoc)
			{
				var vsdocFileContents = _VsDocFileTemplate.Replace("#KEYS#", vsdocSb.ToString().TrimEnd(',')).Replace("#REGISTERNAMESPACE#", registerNamespace).Replace("#NAMESPACE#", resourceNamespaceWithCulture);
